...
<?php 
    ...
    $cart_query = "SELECT * FROM cart WHERE item_id = $item_id AND id = $client_id";
    ...
    if($row_count > 0){
        $update_query = "UPDATE cart SET item_quantity = item_quantity+1 WHERE item_id = $item_id AND id = $client_id";
                ...
                <thead>
                    <tr>
                      ...
                      <?php 
                            ...
                            $cart_query = "SELECT * FROM cart WHERE id = $client_id";
                            $cart_search_query = mysqli_query($db,$cart_query);
                            while ($row = mysqli_fetch_array($cart_search_query)) {
                               ...
                               <td><?php echo $cart_id ?></td>
                                ...
                                <td><img class= 'img-responsive' src='../img/<?php echo $item_image ?>' alt=''></td>";
                               <td><?php echo $item_price ?></td>";
                               <td><?php echo $item_quantity ?></td>";
                               <td><?php echo $total ?></td>";
                               <td><a href='cart.php?add=$cart_id&user=$client_id'>Add</a></td>
                                <td><a href='cart.php?reduce=$cart_id&user=$client_id'>Reduce</a></td>
                            ...
                            if (isset($_GET['remove'])) {
                                ...
                                $remove_query = "DELETE FROM cart WHERE item_id = $removed_item_id AND id = $client_id";
                            ...
                            if (isset($_GET['add'])) {
                                ...
                                $add_item_query = "UPDATE cart SET item_quantity = item_quantity + 1 WHERE item_id = $added_item_id AND id = $client_id";
                            ...
                            if (isset($_GET['reduce'])) {
                                ...
                                $check_query = "SELECT * FROM cart WHERE item_id = $reduced_item_id AND id = $client_id ";
                                ...
                                if ($quantity == 1 ) {
                                    $reduce_item_query = "DELETE FROM cart WHERE item_id = $reduced_item_id AND id = $client_id";
                                    $reduced_item_query = mysqli_query($db,$reduce_item_query);
                                }else{
                                    $reduce_item_query = "UPDATE cart SET item_quantity = item_quantity - 1 WHERE item_id = $reduced_item_id AND id = $client_id";
                                    $reduced_item_query = mysqli_query($db,$reduce_item_query);
                                }
